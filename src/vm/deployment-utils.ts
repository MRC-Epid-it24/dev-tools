...
export class DeploymentUtils {
    ...
    private readonly exampleInstanceDirectoryPath: string;
    private readonly buildInstanceDirectoryPath: string;
    private readonly settings: Settings;
    private readonly logger: Logger;

    constructor(config: Settings, buildId: string, logger: Logger) {
        this.settings = config;
        this.buildId = buildId;
        this.logger = logger;
        this.exampleInstanceDirectoryPath = path.resolve(this.settings.deployment.directory, 'instances', 'example');
        this.buildInstanceDirectoryPath = path.resolve(this.settings.deployment.directory, 'instances', buildId);
    }
    ...
    async execDeploymentScript(name: string): Promise<void> {
        return await execDisplayOutput(path.resolve(this.settings.deployment.directory, name), [this.buildId], this.settings.deployment.directory);
    }
